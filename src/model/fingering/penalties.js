...
export function fingersCrossingPenalty(trackFingering) {
    let penalty = 0;
...
//   return penalty;
// }
export function stringFretVelocityPenalty(trackFingering) {
    let penalty = 0;
    for (let i = 0; i < trackFingering.length - 1; i++) {
        const currentFingering = trackFingering[i].fingering;
        const nextFingering = trackFingering[i + 1].fingering;
...
export function fingerVelocityPenalty(trackFingering) {
    let penalty = 0;
    for (let i = 0; i < trackFingering.length - 1; i++) {
        const currentFingering = trackFingering[i].fingering;
        const nextFingering = trackFingering[i + 1].fingering;
        if (currentFingering.fret === 0 || nextFingering.fret === 0)
            continue;
        if (currentFingering.finger !== nextFingering.finger)
            continue;
        const distance = Math.abs(currentFingering.fret - nextFingering.fret) * 5 + Math.abs(currentFingering.string - nextFingering.string);
        const time = trackFingering[i + 1].startTimeMs - trackFingering[i].startTimeMs;
        penalty += distance / time;
...
export function fingerStringJumpingPenalty(trackFingering) {
    let penalty = 0;
    let fingersPositions = {};
...
export function handMovementPenalty(trackFingering) {
    let penalty = 0;
    let movement = 0;
    for (let i = 0; i < trackFingering.length - 1; i++) {
...
// exit process
// process.exit(0);
export function computeComplexity(trackFingering) {
...
export function reportComplexity(trackFingering) {
    return {
...
;
export function setupSoftConstraints(trackFingering) {
    const softConstraints = [
        ...
        new WeightedSoftConstraint(new SimpleSoftConstraint((trackFingering) => fingerVelocityPenalty(trackFingering), trackFingering), 1),
        new WeightedSoftConstraint(new SimpleSoftConstraint((trackFingering) => fingerStringJumpingPenalty(trackFingering), trackFingering), 1),
