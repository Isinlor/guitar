...
describe('HandMovementPenalty', () => {
    ...
    const createFingering = (string, fret, finger) => ({
        string,
        fret,
        finger
    });
    const createNoteEventWithFingering = (index, [string, fret, finger]) => ({
    ...
    const createTrackFingering = (fingerings) => fingerings.map((fingering, index) => createNoteEventWithFingering(index, fingering));
    ...
    const testTransition = (description, from, to, expectedPenalty) => {
        it(description, () => {
    ...
    testTransition("fret 1 finger 1 -> fret 2 (+1) finger 2 (+1) =0", [1, 1, 1], [1, 2, 2], 0);
    testTransition("fret 1 finger 1 -> fret 4 (+3) finger 4 (+3)", [1, 1, 1], [1, 4, 4], 0);
    testTransition("fret 1 finger 1 -> fret 2 (+1) finger 1 (0)", [1, 1, 1], [1, 2, 1], 7);
    testTransition("fret 1 finger 1 -> fret 1 (0) finger 2 (+1)", [1, 1, 1], [1, 1, 2], 7);
    testTransition("fret 1 finger 1 -> fret 12 (+11) finger 1 (0)", [1, 1, 1], [1, 12, 1], 127);
    testTransition("fret 1 finger 1 -> fret 7 (+6) finger 1 (0)", [1, 1, 1], [1, 7, 1], 42);
    testTransition("fret 1 finger 1 -> fret 2 (+1) finger 3 (+2)", [1, 1, 1], [1, 2, 3], 7);
    testTransition("fret 1 finger 1 -> fret 12 (+11) finger 4 (+3)", [1, 1, 1], [1, 12, 4], 70);
    ...
    testTransition("fret 8 finger 1 -> fret 1 (-7) finger 1 (0)", [1, 8, 1], [1, 1, 1], 55);
});
...
function createNoteEvent(fingering) {
    return {
