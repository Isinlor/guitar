export default function autoCorrelate(buf, sampleRate, expectedFrequency = null) {
    ...
    for (var i = 0; i < SIZE / 2; i++)
        if (Math.abs(buf[i]) < thres) {
            r1 = i;
            break;
        } // Set r1 to the index where signal exceeds the threshold
    for (var i = 1; i < SIZE / 2; i++)
        if (Math.abs(buf[SIZE - i]) < thres) {
            r2 = SIZE - i;
            break;
        } // Set r2 to the index where signal falls below the threshold
    ...
    var d = 0;
    while (c[d] > c[d + 1])
        d++; // Find the first drop in autocorrelation values
    ...
    if (a)
        T0 = T0 - b / (2 * a); // Adjust T0 using the vertex formula
