export default function autoCorrelate(buf: Float32Array, sampleRate: number, expectedFrequency: number | null = null) {
  ...
  let SIZE = buf.length; // Total size of the input buffer
  let rms = 0; // Variable to store the root mean square of the buffer
  ...
  const c = new Array(SIZE).fill(0); // Array to store autocorrelation results
  ...
  let d = 0; while (c[d] > c[d + 1]) d++; // Find the first drop in autocorrelation values
  ...
  let T0 = maxpos; // The estimated pitch period in samples

  let expectedPositionRank = 0;
  if (expectedFrequency) {
    const expectedPositionA = Math.floor(sampleRate / expectedFrequency);
    const expectedPositionB = Math.ceil(sampleRate / expectedFrequency);
    const expectedPositionAValue = c[expectedPositionA];
    const expectedPositionBValue = c[expectedPositionB];
    for (let i = d; i < SIZE; i++) {
  ...
  const a = (x1 + x3 - 2 * x2) / 2; // Coefficient of the quadratic term
  const b = (x3 - x1) / 2; // Coefficient of the linear term
