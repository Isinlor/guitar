...
import fs from 'fs';
export function readMidiFile(filename) {
    return new Midi(fs.readFileSync(filename));
}
export function writeMidiFile(midi, filename) {
    fs.writeFileSync(filename, Buffer.from(midi.toArray()));
}
export function transpose(track, transposition) {
    return track.notes.map(note => {
        return Object.assign(Object.assign({}, note), { midi: note.midi + transposition });
    });
}
export function getNoteEvents(track) {
    return track.notes.map(note => ({
        note: note.midi,
 ...
 */
export function noteNumberFromFrequency(frequency) {
...
*/
export function centsOffFromNote(frequency, note) {
